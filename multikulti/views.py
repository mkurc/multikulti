...
def job_status(jid):
    ...
    if system_info['status'] == 'error':
        flash('Something went wrong. In most cases your PDB file is not properly formatted: <ul><li>continuous residue numbering (breaks are allowed, but chain must be numbered from N to M, where N&lt;M)</li><li>proper atom names within residues (for example if PDB file contains MET residue - atoms must be as in the methionine)</li><li>residue numbering must be unique - you cannot upload structure with the same residue index within the same chain</li><li>each chain must be defined with unique chain index. Empty string (" ") in chain index column is chain index, too. If you uploaded multichain structure without chain index column - you\'ll get an error</li><li>If PDB file looks good, do not hesistate to contact us (email in <a href="'+url_for('index_contact')+'">Contact tab</a>), we will check logs of your job.</li></ul>', 'error')
